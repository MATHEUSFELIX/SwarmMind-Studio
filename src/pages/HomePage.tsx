...
function SimCard({ sim }: { sim: Simulation }) {
  ...
  return (
    <Link to={`/simulation/${sim.id}`} className="card-hover p-4 flex items-center gap-3 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-400 truncate mt-0.5">{sim.scenario.slice(0, 60)}…</p>
      </div>
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        ...
        <span className="text-xs text-gray-400">{sim.messages.length} messages</span>
      </div>
    </Link>
  )
}
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard
          ...
          sub="total configured" icon="🤖" accent="purple"
        />
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <Link
              key={mode}
              to={`/new?mode=${mode}`}
              className={`bg-white border ${border} ${hover} rounded-xl p-4 transition-all duration-200 hover:shadow-md group`}
            >
