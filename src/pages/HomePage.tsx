import { Link } from 'react-router-dom'
import { Plus, Brain, Users, BookOpen, Shirt, Zap, Clock, CheckCircle, Loader2, Circle } from 'lucide-react'
import { useSimulationStore } from '@/stores/simulationStore'
import MetricCard from '@/components/MetricCard'
import type { Simulation } from '@/types'

const MODE_CONFIG = {
  consulting: { icon: Brain, label: 'Consulting', color: 'text-blue-400', bg: 'bg-blue-500/10' },
  social: { icon: Users, label: 'Social', color: 'text-pink-400', bg: 'bg-pink-500/10' },
  research: { icon: BookOpen, label: 'Research', color: 'text-purple-400', bg: 'bg-purple-500/10' },
  fashion: { icon: Shirt, label: 'Fashion', color: 'text-fuchsia-400', bg: 'bg-fuchsia-500/10' },
}

const STATUS_CONFIG = {
  completed: { icon: CheckCircle, label: 'Completed', cls: 'badge-green' },
  running: { icon: Loader2, label: 'Running', cls: 'badge-amber' },
  idle: { icon: Circle, label: 'Idle', cls: 'badge-slate' },
  error: { icon: Circle, label: 'Error', cls: 'badge-red' },
}

function SimCard({ sim }: { sim: Simulation }) {
  const mode = MODE_CONFIG[sim.mode]
  const ModeIcon = mode.icon
  const status = STATUS_CONFIG[sim.status]
  const StatusIcon = status.icon

  return (
    <Link
      to={`/simulation/${sim.id}`}
      className="card-hover p-4 flex items-center gap-3 group"
    >
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${mode.bg}`}>
        <ModeIcon size={18} className={mode.color} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-slate-200 truncate group-hover:text-white">
          {sim.name}
        </p>
        <p className="text-xs text-slate-500 truncate mt-0.5">{sim.scenario.slice(0, 60)}…</p>
      </div>
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        <span className={`badge ${status.cls}`}>
          <StatusIcon size={10} className={sim.status === 'running' ? 'animate-spin' : ''} />
          {status.label}
        </span>
        <span className="text-xs text-slate-600">
          {sim.messages.length} messages
        </span>
      </div>
    </Link>
  )
}

export default function HomePage() {
  const { simulations } = useSimulationStore()

  const total = simulations.length
  const completed = simulations.filter((s) => s.status === 'completed').length
  const running = simulations.filter((s) => s.status === 'running').length

  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {/* Header */}
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Command Center</h1>
          <p className="text-slate-500 text-sm mt-1">Multi-agent swarm intelligence platform</p>
        </div>
        <Link to="/new" className="btn-primary flex items-center gap-2">
          <Plus size={16} />
          New Simulation
        </Link>
      </div>

      {/* Metrics */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <MetricCard label="Total" value={total} sub="all simulations" icon="🧠" accent="amber" />
        <MetricCard label="Completed" value={completed} sub="finished runs" icon="✓" accent="green" />
        <MetricCard label="Running" value={running} sub="active now" icon="⚡" accent="blue" />
        <MetricCard
          label="Agents"
          value={simulations.reduce((a, s) => a + s.agents.length, 0)}
          sub="total configured"
          icon="🤖"
          accent="purple"
        />
      </div>

      {/* Quick Start */}
      <div className="mb-8">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">
          Quick Start
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            {
              mode: 'consulting',
              title: 'Consulting',
              desc: 'Strategic scenario & agent debate',
              icon: Brain,
              color: 'text-blue-400',
              bg: 'bg-blue-500/10 border-blue-500/20',
            },
            {
              mode: 'social',
              title: 'Social',
              desc: 'Opinion dynamics & sentiment',
              icon: Users,
              color: 'text-pink-400',
              bg: 'bg-pink-500/10 border-pink-500/20',
            },
            {
              mode: 'research',
              title: 'Research',
              desc: 'Clustering, themes & synthesis',
              icon: BookOpen,
              color: 'text-purple-400',
              bg: 'bg-purple-500/10 border-purple-500/20',
            },
            {
              mode: 'fashion',
              title: 'Fashion',
              desc: 'Collection & licensing intelligence',
              icon: Shirt,
              color: 'text-fuchsia-400',
              bg: 'bg-fuchsia-500/10 border-fuchsia-500/20',
            },
          ].map(({ mode, title, desc, icon: Icon, color, bg }) => (
            <Link
              key={mode}
              to={`/new?mode=${mode}`}
              className={`card border p-4 hover:scale-[1.02] transition-all duration-200 ${bg}`}
            >
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg ${bg}`}>
                  <Icon size={20} className={color} />
                </div>
                <div>
                  <p className={`font-semibold text-sm ${color}`}>{title}</p>
                  <p className="text-xs text-slate-500 mt-0.5">{desc}</p>
                </div>
              </div>
            </Link>
          ))}
        </div>
      </div>

      {/* Recent Simulations */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
            Recent Simulations
          </p>
          <Clock size={12} className="text-slate-600" />
        </div>

        {simulations.length === 0 ? (
          <div className="card p-12 flex flex-col items-center justify-center text-center">
            <Zap size={32} className="text-slate-700 mb-3" />
            <p className="text-slate-400 font-medium">No simulations yet</p>
            <p className="text-slate-600 text-sm mt-1">Create your first simulation to get started</p>
            <Link to="/new" className="btn-primary mt-4">
              Start a Simulation
            </Link>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {simulations.slice(0, 10).map((sim) => (
              <SimCard key={sim.id} sim={sim} />
            ))}
          </div>
        )}
      </div>
    </div>
  )
}
